...
import { categoryAdd, messageClear, get_category, categoryEdit, categoryDelete } from '../../store/Reducers/categoryReducer'
const Category = () => {
    ...
    const [imageShow2, setImageShow] = useState('');

    const [state, setState] = useState({
    name: editingCategory ? editingCategory.name : '',
    image: editingCategory ? editingCategory.image : ''
});
...
const imageHandle = (e) => {
    ...
    if (files.length > 0) {
        ...
        setImageShow(URL.createObjectURL(files[0])); // Update imageShow state
    }
};

console.log(imageShow, 'hhhhhwww');
    const handleDeleteCategory = async (categoryId) => {
        try {
            await dispatch(categoryDelete(categoryId));
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (editMode) {
            dispatch(categoryEdit({ id: editingCategory._id, updatedData: state }));
        } else {
                                        ...
                                        categorys.map((d, i) => <tr key={i}>
                                            ...
                                            <td  className='py-1 px-4 font-medium whitespace-nowrap'>
                                                ...
                                                <button className='p-[6px] bg-yellow-500 rounded hover:shadow-lg hover:shadow-yellow-500/50' onClick={() => {
                                                    ...
                                                    <button className='p-[6px] bg-red-500 rounded hover:shadow-lg hover:shadow-red-500/50' onClick={() => handleDeleteCategory(d._id)} ><FaTrash /></button>
                                                </div>
                                            </td>
                                        </tr>)
                                    }
                    ...
                    <div className='w-full pl-5'>
                        <div className='bg-[#283046] h-screen lg:h-auto px-3 py-2 lg:rounded-md text-[#d0d2d6]'>
                            <div className='flex justify-between items-center mb-4'>
                                ...
                                <div onClick={() => setShow(false)} className='block lg:hidden cursor-pointer'><GrClose className='text-[#d0d2d6]' />{state?.name}</div>
                            </div>
                            ...
                            <div className='flex flex-col w-full gap-1 mb-3'>
                                ...
                                />
                                {imageShow2 && (
                                    <img className='w-[45px] h-[45px]' src={imageShow2} alt="Selected" />
                                )}
                            </div>
                                <div>
                                ...
                                <div className='mt-4'>
                                    <button disabled={loader ? true : false} className='bg-blue-500 w-full hover:shadow-blue-500/20 hover:shadow-lg text-white rounded-md px-7 py-2 mb-3'>
                                        {
                                            loader ? <PropagateLoader color='#fff' cssOverride={overrideStyle} /> : editMode ? 'Update Category' : 'Add Category'
                                        }
                                    </button>
                                </div>
                            </form>
